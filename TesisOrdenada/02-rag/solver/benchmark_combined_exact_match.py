...
logger.add("benchmark_combined_exact_match_logs.log", rotation="500 MB")
...
async def evaluate_correctness(
    ...
    usando exclusivamente el LLM para el juicio. Solo considera la parte antes de "Explanation:"
    """
    try:
        # Extraer solo la respuesta antes de "Explanation:"
...
async def generate_combined_rag_response(
    ...
    """
    Genera una respuesta con Combined RAG y captura los tiempos
    ...
    try:
        ...
        # Configurar parámetros para Combined RAG
        params = {
            "query": question,
        ...
        # Ejecutar Combined RAG y obtener respuesta y tiempo total
        ...
        logger.error(f"Error ejecutando Combined RAG: {e}")
...
async def run_benchmark(
    ...
    # Crear directorio para respuestas exactas
    ...
    if remaining_questions == 0:
        ...
        await generate_stats_report(exact_results, exact_dir, "Combined RAG Results")
    ...
    for i, question_data in enumerate(tqdm(questions_to_process, desc="Procesando preguntas")):
        ...
        # Generar respuesta con RAG combinado
        response, total_time, retrieval_time = await generate_combined_rag_response(
        ...
        if "answer" in question_data:
            ...
            try:
                if isinstance(reference_answer, list):
                    ...
                    exact_match = max([calculate_exact_match(response, str(ref)) for ref in reference_answer]) if reference_answer else 0
                else:
                    ...
                    exact_match = calculate_exact_match(response, reference_answer)
            except Exception as e:
...
async def generate_stats_report(results: List[Dict[str, Any]], output_dir: Path, report_title: str):
    ...
    with open(output_dir / "stats_report.txt", "w", encoding="utf-8") as f:
        ...
        f.write(f"Respuestas correctas (LLM): {correct_answers}\n")
        f.write(f"Exact matches: {exact_matches}\n")
        f.write(f"Precisión (accuracy): {accuracy:.2%}\n")
        f.write(f"Exact match rate: {exact_match_rate:.2%}\n\n")
