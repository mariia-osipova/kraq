...
logger.add("benchmark_traditional_exact_match_logs.log", rotation="500 MB")
...
async def run_benchmark(
    ...
    questions_to_process = [q for q in all_questions if q.get("question_id", "") not in processed_question_ids]
    ...
    if remaining_questions == 0:
        ...
        await generate_stats_report(answer_results, answer_dir, "Traditional RAG Results")
        
        logger.info(f"Benchmark completado. Resultados guardados en {results_dir}")
        return
    
    # Procesar cada pregunta restante
    for i, question_data in enumerate(tqdm(questions_to_process, desc="Evaluando preguntas")):
        question = question_data["question"]
        question_id = question_data.get("question_id", "unknown")
    
        logger.info(f"Procesando pregunta {i+1}/{len(questions_to_process)} (ID: {question_id}): {question[:50]}...")
        ...
        if "answer" in question_data:
            ...
            if has_response:
                ...
                exact_match = calculate_exact_match(response, reference)
