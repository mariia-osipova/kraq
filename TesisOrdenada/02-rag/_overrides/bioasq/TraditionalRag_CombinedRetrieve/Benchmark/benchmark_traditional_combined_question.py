...
async def generate_traditional_rag_response(
    ...
    try:
        ...
        class TimeCaptureLogger:
            ...
            def write(self, text):
                ...
                if "retrieval_time:" in text:
                    try:
                        # Buscar el patrón "retrieval_time:X.XXXX"
                        match = re.search(r"retrieval_time:(\d+\.\d+)", text)
                        if match:
                            self.times["retrieval_time"] = float(match.group(1))
                    except Exception as e:
                        logger.error(f"Error al extraer retrieval_time: {e}")
                return
        ...
        try:
            ...
            # Restaurar sys.stdout antes de loggear el error
            sys.stdout = original_stdout
            logger.error(f"Error ejecutando Traditional RAG: {e}")
            # Si hay un error crítico durante la ejecución, devolver valores predeterminados
            return "Error generando respuesta", time.time() - start_time, 0
        ...
        # Añadir una guarda por si el retrieval_time no es un número válido
        if not isinstance(retrieval_time, (int, float)):
            logger.warning(f"retrieval_time no es un número válido: {retrieval_time}")
            retrieval_time = 0
            
        return response, total_time, retrieval_time
        
    except Exception as e:
        # Asegurar que sys.stdout se restaura incluso en caso de error
