...
async def run_benchmark_with_qdrant(limit: int = 1000, results_dir: Optional[Path] = None, **kwargs):
    """
    ...
    Args:
        ...
        **kwargs: Parámetros adicionales (k, m, top_k, etc.)
    """
        ...
        if "answer" in question_data or "ideal_answer" in question_data or "exact_answer" in question_data:
            try:
                ...
                params = {
                    ...
                    "seed": 23,
                    "client": ollama_client, # Cliente genérico (Ollama para embeddings)
                    "drafter_model": DRAFTER_MODEL,
                    "verifier_model": VERIFIER_MODEL,
                    "m": 5,  # Número de subsets de documentos
                    "top_k": 10,  # Documentos a recuperar
                    "verbose": False, # Mantener en False para no llenar logs
                    "debug_prints": True, # True para capturar tiempos con TimeCaptureLogger
...
if __name__ == "__main__":
    ...
    parser.add_argument("--limit", type=int, default=1000, help="Número máximo de preguntas a evaluar (default: 1000)")
