...
async def rag_drafting_generator(
    ...
    prompt = """Given the following medical claim and evidence, determine whether it is true, false, or a mixture of both. Then, provide a brief rationale explaining your reasoning..
...
async def rag_verifier_generator(
    ...
    """
    Verify RAG drafts sequentially and return probabilities
    """
    # Si no se proporciona cliente, crear uno que apunte a vLLM local
    if client is None:
        client = AsyncOpenAI(
...
Do not include any explanation or additional text."""
    ...
    # Ejecutar todas las verificaciones secuencialmente y medir tiempos individuales
    verifications = []
    verification_times = []
    ...
    # Return best draft and its probability along with timing info
...
async def speculative_rag_with_probabilities(
    ...
    Args:
        ...
        drafter_client: Cliente específico para el drafter (si es None, usa client)
        verifier_client: Cliente específico para el verifier (si es None, usa client)
    ...
    # Si no se proporcionan clientes específicos, usar el cliente principal
