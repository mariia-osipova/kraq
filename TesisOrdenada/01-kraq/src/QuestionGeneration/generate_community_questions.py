...
def save_progress(questions_list: List[Dict], level: int, is_checkpoint=False):
    ...
    if is_checkpoint:
        ...
        checkpoint_files = [f for f in os.listdir(checkpoint_dir) 
                          if f.startswith(f"community_questions_level_{level}_checkpoint_") and f.endswith(".json")]
        ...
        temp_path = os.path.join(checkpoint_dir, f"community_questions_level_{level}_checkpoint_{timestamp}.json")
    else:
        temp_path = os.path.join(output_dir, f"temp_questions_level_{level}.json")
    ...
    try:
        ...
        try:
            ...
            recovery_path = os.path.join(output_dir, f"recovery_questions_level_{level}_{timestamp}.json")
    ...
    if finding_messages_list:
        ...
        for i in range(0, len(finding_messages_list), batch_size):
            ...
            try:
                ...
                finding_questions = generate_batch(finding_prompts, batch_size=2)
                
                for community_id, explanation, question in zip(batch_community_ids, batch_explanations, finding_questions):
                    try:
...
def save_questions_for_level(target_level: int, max_main_questions: int = None, max_finding_questions: int = None, 
    ...
    try:
        ...
        output_path = os.path.join(output_dir, f"community_questions_level_{target_level}.json")
