...
def save_progress(questions_list: List[Dict], level: int, is_checkpoint=False):
    ...
    if is_checkpoint:
        ...
        checkpoint_files = [f for f in os.listdir(checkpoint_dir) 
                          if f.startswith(f"community_questions_base_main_only_level_{level}_checkpoint_") and f.endswith(".json")]
        ...
        temp_path = os.path.join(checkpoint_dir, f"community_questions_base_main_only_level_{level}_checkpoint_{timestamp}.json")
    else:
        temp_path = os.path.join(output_dir, f"temp_questions_base_main_only_level_{level}.json")
    ...
    try:
        ...
        try:
            ...
            recovery_path = os.path.join(output_dir, f"recovery_questions_base_main_only_level_{level}_{timestamp}.json")
...
def generate_questions_for_level(target_level: int, max_main_questions: int = None,
    ...
    # Definir el prompt para preguntas principales
...
def save_questions_for_level(target_level: int, max_main_questions: int = None,
    ...
    try:
        ...
        output_path = os.path.join(output_dir, f"community_questions_base_main_only_level_{target_level}.json")
