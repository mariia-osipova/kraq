...
from src.QuestionGeneration.llm_inference import generate_batch
...
def save_progress(questions_list: List[Dict], level: int, is_checkpoint=False):
    ...
    if is_checkpoint:
        ...
        checkpoint_files = [f for f in os.listdir(checkpoint_dir) 
                          if f.startswith(f"community_questions_main_only_level_{level}_checkpoint_") and f.endswith(".json")]
        ...
        temp_path = os.path.join(checkpoint_dir, f"community_questions_main_only_level_{level}_checkpoint_{timestamp}.json")
    else:
        temp_path = os.path.join(output_dir, f"temp_questions_main_only_level_{level}.json")
    ...
    try:
        ...
        try:
            ...
            recovery_path = os.path.join(output_dir, f"recovery_questions_main_only_level_{level}_{timestamp}.json")
...
def generate_questions_for_level(target_level: int, max_main_questions: int = None,
    ...
    # Usar todos los datos disponibles si max_main_questions es None
...
"""}
    ...
    # Generar preguntas principales en lotes más pequeños para permitir checkpoints
    if main_messages_list:
        print(f"\nGenerando {len(main_messages_list)} preguntas principales nuevas...")
        
        # Procesar en lotes para permitir checkpoints frecuentes
        batch_size = min(10, checkpoint_interval)  # Usar lotes de 10 o menos
        batch_count = 0
        ...
        for i in range(0, len(main_messages_list), batch_size):
            ...
            try:
                ...
                generated_questions = generate_batch(main_prompts, batch_size=2)
                
                for community_id, summary, question in zip(batch_community_ids, batch_summaries, generated_questions):
                    try:
...
def save_questions_for_level(target_level: int, max_main_questions: int = None,
                           checkpoint_interval: int = 50, resume_from_checkpoint: bool = True):
    start_time = time.time()
    mensaje_limites = "Iniciando generación de TODAS las preguntas principales para nivel"
    if max_main_questions is not None:
        mensaje_limites = f"Iniciando generación de preguntas para nivel {target_level} (máximo {max_main_questions} principales)..."
    else:
        mensaje_limites = f"Iniciando generación de TODAS las preguntas principales para nivel {target_level}..."
    ...
    try:
        ...
        output_path = os.path.join(output_dir, f"community_questions_main_only_level_{target_level}.json")
