...
# Ya no necesitamos importar generate_batch ya que usaremos preguntas existentes
...
def get_tokenizer():
    ...
    if _tokenizer is None:
        from src.QuestionGeneration.llm_inference import BASE_MODEL_ID
...
# Ya no necesitamos esta función ya que usaremos preguntas existentes de la colección random
# def generate_question_from_fragments(fragments):
#     # ...
...
def generate_and_analyze_questions(num_iterations=1000, collection_name=None, random_collection=None):
    """
    Compara preguntas de la colección random con las almacenadas en la colección finetuned
    usando la búsqueda vectorial de Qdrant.
    """
    # Usar las colecciones específicas o las predeterminadas
    finetuned_collection = collection_name or COLLECTIONS["finetuned"]
    ...
    print("\nAnalizando similitud entre preguntas random y finetuned...")
    for i, (random_question, random_id) in enumerate(tqdm(zip(selected_random_questions, selected_random_ids), total=len(selected_random_questions), desc="Procesando preguntas")):
        ...
        search_result = client.search(
            collection_name=finetuned_collection,
    ...
    if resultados_detallados:
        ...
        # Guardar resultados
        output_path = os.path.join(project_root, "output", "analisis_preguntas_random_vs_finetuned.json")
...
if __name__ == "__main__":
    print("Comenzando análisis de preguntas random vs. finetuned")
    generate_and_analyze_questions(num_iterations=1000)
