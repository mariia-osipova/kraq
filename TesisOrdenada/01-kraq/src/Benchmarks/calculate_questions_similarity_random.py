# src/Benchmarks/calculate_questions_similarity_random.py
import json
import os
import sys
import numpy as np
...
# Nombre de la colección específica para preguntas aleatorias
...
def calculate_and_save_proximities():
    """Calcula la similitud entre preguntas del benchmark y las almacenadas en Qdrant (preguntas random)."""
    print("=== Iniciando proceso de similitud (Preguntas Random) ===")
    ...
    try:
        ...
        if resultados:
            ...
            # Estadísticas de número de fragmentos usados
            if resultados and 'fragmentos' in resultados[0]:
                num_fragmentos = [len(r.get('fragmentos', [])) for r in resultados if 'fragmentos' in r]
                if num_fragmentos:
                    info_fragmentos = {
                        "promedio_fragmentos_por_pregunta": float(np.mean(num_fragmentos)),
                        "min_fragmentos": int(np.min(num_fragmentos)),
                        "max_fragmentos": int(np.max(num_fragmentos)),
                        "total_preguntas_con_fragmentos": len(num_fragmentos)
                    }
                    info_coleccion["estadisticas_fragmentos"] = info_fragmentos
            ...
            # Guardar resultados
            output_path = os.path.join(project_root, "output", "similitudes_random.json")
            ...
            print("\n=== Estadísticas de Similitud (Preguntas Random) ===")
...
if __name__ == "__main__":
    print("Comenzando benchmark para preguntas aleatorias (questions-index-random)")
